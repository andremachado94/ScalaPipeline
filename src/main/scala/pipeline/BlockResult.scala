...
case class BlockResult[T](result: Option[T], blockName: String, executionStartTime: LocalDateTime, exception: Option[Throwable] = None){
  ...
  private val executionStatus =
    if(result.isDefined && exception.isEmpty) PipelineConstants.METRICS_CODE_SUCCESS_CODE
    else if(result.isEmpty && exception.isDefined) PipelineConstants.METRICS_CODE_FAILURE_CODE
    else PipelineConstants.METRICS_CODE_IMPLEMENTATION_ERROR_CODE
  ...
  def successfulExecution: Boolean = executionStatus.equals(PipelineConstants.METRICS_CODE_SUCCESS_CODE)
